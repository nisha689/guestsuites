...
class IndexController extends Controller
{
    ...
    public function index( Request $request )
    {
        ...
        $businessId = ! empty( $data['business_id'] ) ? $data['business_id'] : "";
        ...
        $businessDropDown = $this->userObj->getBusinessDropDown('Businesses');
        $businessDropDownForAdd = $this->userObj->getBusinessDropDown('Select Business');
         
        $businessName = "";
        if($businessId > 0){
            $businessName = $this->userObj->getDateById($businessId)->company_name;
        }

        return view( 'admin.customer.index', compact( 'sortedBy', 'sortedOrder', 'recordStart', 'businesses', 'paging', 'totalRecordCount', 'name', 'email', 'statusDropDown', 'status', 'createdStartDate', 'createdEndDate','countryDropDown','businessDropDown','businessDropDownForAdd','businessId','businessName' ) );
    }
    ...
    public function delete( Request $request )
    {
        ...
        if ( $isDelete ) {
            ...
            $request->session()
                    ->flash( 'error', 'Customer is not deleted successfully.' );
        }
        if($isListingPage){
    ...
    public function saveAjax( Request $request )
    {
        ...
        $data['status'] = 1;
        $data['role_id'] = $this->_helperRoles->getCustomeRoleId();
        $data['plan_id'] = 3;
    ...
    public function profileDetails( Request $request, $customerId )
    {
        $customerId = Common::getDecryptId( $customerId );

        if ( $customerId <= 0 ) {
            return abort( 404 );
        }

        $business = $this->userObj->getDateById( $customerId );
        if ( ! Common::isCustomer( $business ) ) {
            return abort( 404 );
        }
        ...
        return view( 'admin.customer.profile', compact( 'business','countryDropDown','stateDropDown','cityDropDown','businessDropDownForAdd') );
    }
    ...
    public function changePassword( Request $request )
    {
        ...
        if ( ! empty( $result['user_id'] ) && $result['user_id'] > 0 ) {
            ...
            $entity = "customer_password_change";
            $emailTemplate = $this->emailTemplateObj->getDateByEntity( $entity );
            $templateFields = $emailTemplate->template_fields;
            $templateContent = $emailTemplate->template_content;
    ...
    public function profileSave( Request $request )
    {
        ...
        if ( ! empty( $result['user_id'] ) && $result['user_id'] > 0 ) {
            $request->session()
                    ->flash( 'success', 'Customer profile update successfully.' );
            return Redirect::back();
        } else {
            return Redirect::back()
                           ->withErrors( $result['message'] );
